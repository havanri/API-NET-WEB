    ...
    public class ProductService : IProductService
    {
        ...
        public async Task<bool> CreateProductAsync(CreateProductDto createProductDto)
        {
            ...
            try
            {
                ...
                if (createProductDto.formFile != null)
                {
                    product.FeatureImageName = createProductDto.formFile.FileName;
                    var filename = Guid.NewGuid().ToString() + ".png";
                    var path = Path.Combine(_webHostEnvironment.WebRootPath, "images", filename);
                    var stream = System.IO.File.Create(path);
                    createProductDto.formFile.CopyTo(stream);
                    stream.Close();
                    product.FeatureImagePath = Path.Combine("images", filename);
                }
        ...
        public async Task<bool> UpdateProductAsync(int productId, UpdateProductDto updateProductDto)
        {
            ...
            try
            {
                ...
                product.Slug = StringExtendsions.Slugify(updateProductDto.Name);
                product.UpdatedAt = DateTime.Now;
                if (updateProductDto.formFile != null)
                {
                    product.FeatureImageName = updateProductDto.formFile.FileName;
                    var filename = Guid.NewGuid().ToString() + ".png";
                    var path = Path.Combine(_webHostEnvironment.WebRootPath, "images", filename);
                    var stream = System.IO.File.Create(path);
                    updateProductDto.formFile.CopyTo(stream);
                    stream.Close();
                    product.FeatureImagePath = Path.Combine("images", filename);
                }
